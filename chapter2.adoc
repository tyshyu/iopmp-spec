...
Examples of implementation-dependent behaviors for handling improper MDCFG settings are provided in <<#IMPROPER_MDCFG_REFERENCE,Reference Behaviors of Improper Settings on MDCFG Table>>.
...
The error type (`ERR_INFO.etype`) identifies the cause of an illegal transaction. The error-type encodings are summarized in <<#ERROR_TYPES,Summary of error types>>. For error type = "User-defined error" (0x0E, 0x0F), behavior of interrupt and error response is user-defined.
