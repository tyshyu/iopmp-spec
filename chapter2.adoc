...
* The IOPMP uses the RRID to look up the SRCMD Table and identifies the memory domains associated with the requester.
* The IOPMP uses the associated memory domains to look up the MDCFG Table and identifies the entries belonging to those memory domains.
* The IOPMP applies the entry priority and matching rules below to those entries.
...
The error types referenced in this section are summarized in <<#ERROR_TYPES,Summary of error types>>.
...
The error type (`ERR_INFO.etype`) identifies the cause of an illegal transaction. For error type = "User-defined error" (0x0E, 0x0F), behavior of interrupt and error response is user-defined.
