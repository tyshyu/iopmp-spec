/***************************************************************************
// libiopmp tests for SRCMD_FMT=1 & MDCFG_FMT=2.
//
// RRID s is hardwired to MD s and there is no MDCFG table, but unlike
// Compact-k the entries each MD owns is programmable through
// HWCFG3.md_entry_num until HWCFG0.enable locks it.
***************************************************************************/
...
int main(void)
{
    ...
    START_TEST("Initialize SRCMD_FMT=1 & MDCFG_FMT=2");
    ...
    START_TEST("HWCFG3.md_entry_num is locked by HWCFG0.enable");
    FAIL_IF(iopmp_set_enable(&iopmp) != IOPMP_OK);
    val_u32 = 3;
    FAIL_IF(iopmp_set_md_entry_num(&iopmp, &val_u32) !=
